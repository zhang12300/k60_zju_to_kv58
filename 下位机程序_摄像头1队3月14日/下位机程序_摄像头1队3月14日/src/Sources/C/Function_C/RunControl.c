...
{ 
  ...
  DeviFuse = (DeviFuse*10 + DeviFuse_old*0)/10;//DeviFuse=DeviFuse by zhang 20180912
  DeviFuse_old = DeviFuse;//DeviFuse_old的初始值为零；
...
//};by zhang 20180912
  ...
  //P分量。
  ServoPIDInputValP = ServoPIDP * DeviFuse / 10;//初始值为0；int32 ServoPIDInputValP = 0;            //实际输入到舵机的值的P分量
  //D分量
  
  if((AbsInt(DeviFuse)-AbsInt(DeviPre))>=0)//DeviPre=DeviFuse;DeviationFuse(void)
  {
  ...
  ServoPIDInputValD=ServoPIDD*(DeviFuse-DeviPre);//10*
  //记录上一次的输入值。
  ...
  //滤波
   ServoPIDInputVal = (ServoPIDInputValOld * 0 + ServoPIDInputVal * 10) / 10;//ServePIDInputVal=ServoPIDInputVal       by    zhang 20181002
  /*
   //舵机跳变限幅
  ...
  */
  ...
  //针对障碍的打角限制
  BrickSpeDealServor();////左障碍：强制左转，侧撞总比出线好。
  ...
  //赋值给实际的舵机I/O口。
  //FTM1_C0V = 2100;右打死
  FTM1_C0V = (uint16)(2*ServoPIDCen-ServoPIDInputVal);//#define ServoPIDMax 2110            //舵机PID控制的最大值，右转
                                                      //#define ServoPIDMin 1350            //舵机PID控制的最小值，左转
                                                      //#define ServoPIDCen 1730             //舵机PID控制的中心值，直走。by zhang 20181002
  return 1;
}
...
//   210, 220, 230, 250, 280, 330, 280, 250, 230, 220, 210,    //8档，直道高，弯道高
//};
//int32 DeviFusemtr[11];             //偏差的隶属度//uint8 MtrGet(void)里面有数值
...
//      }
    ...
    //计算左电机应输入的值。
    //储存为上一次左电机的目标速度
    SpeedLastTargetLeftVal = SpeedTargetLeftVal;//extern uint16 SpeedTargetLeftVal;          //左电机目标速度，脉冲数。差速后。by zhang 20181002
                                                //uint16 SpeedTargetLeftVal = 0;           //左电机目标脉冲数。差速后。

    //左电机差速
    if(SpeedDiffGetSwitch == 1)//uint8 SpeedDiffGetSwitch =0;          //差速开关。1开启差速。0关闭差速。

    {
      SpeedTargetLeftVal = SpeedDiffGet(SpeedTargetVal, 0);  ////函数名：SpeedDiffGet
                                                              //功  能：左右电机的差速值计算。
                                                              //参  数：tempSpeedTarget预设速度值，num处理的电机编号0左1右
                                                              //返  回：tempSpeedTarget差速后的预设速度值
                                                              //影  响：
    ...
    //左电机P分量
     temp_speed_bias=AbsInt (SpeedTargetLeftVal - SpeedNowLeft);//extern uint16 SpeedNowLeft;                //左电机实时速度，对应300线编码器，最后转化成m/s并扩大了1000倍
                                                                ...
                                                                //之前调用了在SERVOPID之前，main里面
    
      speed_kd=SpeedPIDDVal;//uint16 SpeedPIDDVal = 65;                   //电机PID选取的D值
